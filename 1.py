...
server = couchdb.Server('http://admin:epn2020@localhost:5984/')
try:
    db = server.create('en_venta')
except:
    db = server['en_venta']

twitterStream.filter(track=['departamentos', 'departamentosenventa', 'casas', 'casasenventa', 'inmobiliaria'])
